...
export default function Hero() {
  return (
    <section
      ...
      aria-label="Hero — Welcome to Check vs Mate Chess Academy"
    >
      {/* Background layers */}
      ...
      <div className="absolute right-8 md:right-16 top-1/2 -translate-y-1/2 text-white/5 font-display select-none pointer-events-none">
        <span style={{ fontSize: "clamp(200px, 30vw, 400px)", lineHeight: 1 }}>
          ♞
        </span>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16 text-center md:text-left">
        <div className="max-w-3xl">
          {/* Badge */}
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-[#C9A84C]/30 rounded-full px-4 py-2 mb-8 animate-fade-in">
            <span className="text-[#C9A84C]">★</span>
            <span className="text-white/90 text-sm font-body font-semibold tracking-wide">
              4.9 Rating · 97+ Google Reviews
            </span>
          </div>

          {/* Headline */}
          <h1 className="font-display font-black text-white leading-tight mb-6 animate-fade-up">
          ...
          >
            Expert coaching for children of all ages — group classes, online
            sessions & home visits. 100+ students trained since 2013.
          </p>
          ...
          >
            ...
            <Link
              href="/about"
              className="px-8 py-4 bg-white/10 backdrop-blur-sm text-white font-body font-semibold text-base rounded tracking-wide border border-white/30 hover:bg-white/20 hover:-translate-y-0.5 transition-all duration-200"
            >
              Learn More
            </Link>
          </div>
          ...
          {/* Stats row */}
          <div
            className="flex flex-wrap gap-8 mt-14 justify-center md:justify-start animate-fade-up"
          ...
          >
            {[
              ...
              <div key={stat.label} className="text-center md:text-left">
                <p className="font-display font-black text-[#C9A84C] text-3xl">
                  {stat.num}
                </p>
                <p className="text-white/60 text-xs font-body uppercase tracking-widest mt-1">
                  {stat.label}
                </p>
              </div>
            ))}
